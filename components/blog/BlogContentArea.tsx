...
import { AdvancedSourceModeView } from '../AdvancedSourceModeView';
...
interface BlogContentAreaProps {
    ...
    onLocalSourceChange: (content: string) => void;
...
export const BlogContentArea: React.FC<BlogContentAreaProps> = ({
    ...
    const isLocalRenderPage = activePath === 'local-render';
    const currentContent = isLocalRenderPage ? localRenderInfo?.content || null : markdownContent;
    const currentFilePath = isLocalRenderPage ? localRenderInfo?.name || 'local-render.md' : activePath || '';
    const isSourceEditable = isLocalRenderPage && !!localRenderInfo;

    const shouldShowToc = viewMode === 'render' &&
        (((activePath && !['source_tree', 'local-render'].includes(activePath) && markdownContent && !isCurrentPathFolder) ||
            (localRenderInfo && isLocalRenderPage)) &&
            isTocOpen);
                ...
                <div className="max-w-4xl mx-auto px-4 py-8 lg:px-12 lg:py-12">
                    {!activePath ? (
                        <div className="flex flex-col items-center justify-center h-[60vh] text-center px-4">
                            ...
                            <p className="mt-4 text-xs text-slate-400 dark:text-slate-500">Or choose a local file to preview and edit it in source mode.</p>
                        </div>
                    ) : activePath === 'source_tree' ? (
                        <SourceTreeView nodes={tree} />
                    ) : isContentLoading && !isLocalRenderPage ? (
                        <LoadingSpinner />
                    ) : !currentContent ? (
                        <LoadingSpinner />
                    ) : (
                        <div className="animate-in fade-in duration-500 slide-in-from-bottom-4">
                            {viewMode === 'source' ? (
                                <AdvancedSourceModeView
                                    content={currentContent}
                                    editable={isSourceEditable}
                                    onChange={isSourceEditable ? onLocalSourceChange : undefined}
                                />
                            ) : (
                                <MarkdownRenderer content={currentContent} filePath={currentFilePath} isFolder={isCurrentPathFolder} />
                            )}
                        </div>
                    )}
